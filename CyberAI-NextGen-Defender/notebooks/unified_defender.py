...
class UnifiedDefender:
    ...
    """
    Load ANY model that follows the pattern:
    threat_model.pkl
    threat_vectorizer.pkl
    threat_scaler.joblib
    threat_label_encoders.joblib
    """
    print("🔍 Scanning models folder:", self.models_path)

    for f in os.listdir(self.models_path):
        full = os.path.join(self.models_path, f)
        fname = os.path.basename(f)

        # split "ddos_model.pkl" -> ["ddos", "model.pkl"]
        parts = fname.split("_", 1)
        if len(parts) < 2:
            continue

        threat = parts[0].lower()
        ...
        if suffix == "model.pkl":
            try:
                ...
                print(f"✅ Loaded model for: {threat}")
            except Exception as e:
                print(f"❌ Failed loading model {fname}: {e}")

        # ------------------ VECTORIZER --------------
        elif suffix == "vectorizer.pkl":
            try:
                self.vectorizers[threat] = joblib.load(full)
                print(f"📌 Loaded vectorizer for: {threat}")
            except:
                pass

        # ------------------ SCALER ------------------
        elif suffix.endswith("scaler.joblib"):
            try:
                self.scalers[threat] = joblib.load(full)
                print(f"📈 Loaded scaler for: {threat}")
            except:
                pass

        # ------------------ LABEL ENCODERS ----------
        elif "label" in suffix and suffix.endswith(".joblib"):
            try:
                self.encoders[threat] = joblib.load(full)
                print(f"🔤 Loaded label encoders for: {threat}")
            except:
                pass

    print("📦 FINAL LOADED MODELS:", list(self.models.keys()))
